🤖 AI-Powered Lead Qualification
https://img.shields.io/badge/license-MIT-blue.svg
https://img.shields.io/badge/built%20with-n8n-EA4B71?logo=n8n&logoColor=white
https://img.shields.io/badge/python-3.10%2B-blue?logo=python&logoColor=white
https://img.shields.io/badge/CI-passing-brightgreen
https://img.shields.io/github/stars/sergeykeba-cell/n8n-ai-automation-leads?style=social
An intelligent automation pipeline that scores and routes sales leads using AI, built with n8n and Python.

📌 Note: replace the CI badge above with the real one generated by https://github.com/sergeykeba-cell/n8n-ai-automation-leads/actions/workflows/ci.yml/badge.svg once GitHub Actions runs for the first time.

<!-- 📸 Add a demo GIF/screenshot here — this is the single highest-impact addition for this README. Record: CSV upload → AI scoring → Telegram alert. Suggested tools: Kap (macOS), ScreenToGif (Windows), peek (Linux). ![demo](docs/screenshots/demo.gif) -->
Problem

Sales teams waste hours manually sorting through raw leads. This system automates the process, so they can focus on the best opportunities immediately.

Solution
Ingest lead data from a CSV file.
Use Groq (or Gemini) to analyze each lead and assign a relevance score.
High-scoring leads are sent instantly to a Telegram chat.
Low-scoring leads are stored for later review.
How it works
CSV upload → n8n trigger → Python (clean + score prompt) → LLM (Groq/Gemini)
                                                              │
                                        ┌─────────────────────┴─────────────────────┐
                                        ▼                                           ▼
                              High score → Telegram alert            Low score → stored for review
Components
Component	Role
n8n workflow (workflows/n8n-ai-automation-leads.json)	Orchestrates the pipeline, triggers on new CSV upload
Python script (scripts/)	Handles data cleaning and AI prompt engineering
Telegram bot	Delivers real-time alerts for high-scoring leads
Tech stack
n8n — workflow automation (tested on n8n 1.6x+)
Python 3.10+ (pandas, requests)
Groq / Gemini API
Telegram Bot API
Quick start (Docker — recommended)
bash
git clone https://github.com/sergeykeba-cell/n8n-ai-automation-leads.git
cd n8n-ai-automation-leads
cp .env.example .env        # fill in your API keys
docker-compose up

n8n will be available at http://localhost:5678. Import the workflow from workflows/n8n-ai-automation-leads.json via Workflows → Import from File.

Manual setup (without Docker)
Clone this repo:
bash
   git clone https://github.com/sergeykeba-cell/n8n-ai-automation-leads.git
   cd n8n-ai-automation-leads
Install Python dependencies:
bash
   pip install -r requirements.txt
Configure your API keys:
bash
   cp .env.example .env
   # then edit .env with your GROQ_API_KEY / TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID
Import the n8n workflow from workflows/n8n-ai-automation-leads.json into your local or cloud n8n instance.
Drop a CSV file into the designated folder — the pipeline runs automatically.

A sample file is provided at data/sample_leads.csv so you can test the pipeline immediately without your own data.

Configuration

All secrets are read from environment variables — see .env.example for the full list.

Contributing

Contributions are welcome! Please read CONTRIBUTING.md before opening a PR, and check open issues for ways to help.

License

Released under the MIT License.

About

Автоматизація обробки лідів з CSV-файлу для бізнесу (CRM, продажі).
